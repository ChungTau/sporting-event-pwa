...
import {RouteFeature, addLayersToMap, calculateDistanceAlongRoute, calculateElevationGainToPoint, findNearestPointOnRoute, getElevationAtPoint, removeAllRoutes, resizeMap} from '../../../../helpers/map';
...
const PlanMap = ({data} : PlanMapProps) => {
  ...
  const [markerData, setMarkerData] = useState<MarkerData>({ id: '', name: '', description: '', services: [], elevation: 0, elevationGain: 0, distance: 0, distanceInter: 0, position: null });
    
  useEffect(() => {
    if (map.mapRef.current) {
      const mapInstance = map.mapRef
        .current
        .getMapInstance();
      if (mapInstance._fullyLoaded) {
        addLayersToMap(map.mapRef, '#887d73', data
          ?.routes);
      }
    }
  }, [
    ...
    if (data && data.routes && data.routes.geometry.coordinates.length > 0) {
      ...
      const endCoord =
        data.routes.geometry.coordinates[
          data.routes.geometry.coordinates.length - 1];
      ...
      const startMarker = createCustomMarker().setLngLat([
        startCoord[0],
        startCoord[1],
      ]);
      const endMarker = createCustomMarker().setLngLat([
        endCoord[0],
        endCoord[1],
      ]);
  ...
  const placePinNearRoute = useCallback(
    (lngLat: Position) => {
      ...
      if (nearest) {
        const newMarker = createCustomMarker()
          ...
          .on('dragend', () => {
            ...
            if (nearestPoint) {
              ...
              const newDistance = calculateDistanceAlongRoute(
                nearestPoint,
                data?.routes as RouteFeature
              );
              const newElevationGain = calculateElevationGainToPoint(
                data?.routes as RouteFeature,
                nearestPoint as [number, number]
              );
              const newElevation = getElevationAtPoint(
                data?.routes as RouteFeature,
                nearestPoint
              );
          ...
          setMarkerData((prev) => ({
            ...
            position: [(lngLat as any).lng, (lngLat as any).lat],
          }));
        ...
        toast({
          title: 'Error',
          description: 'Marker must be placed near the route',
          status: 'error',
          duration: 3000,
          isClosable: true,
        });
      ...
      useEffect(() => {
        ...
        return () => {
          if (canvas) {
            ...
            canvas.removeEventListener('dragover', (event: DragEvent) =>
              event.preventDefault()
            );
          }
        };
    ...
    useEffect(() => {
      ...
      const handleModalSubmit = (checkpointData: MarkerData) => {
        if (tempMarker) {
          ...
          checkpointData.elevationGain = calculateElevationGainToPoint(
            data?.routes as RouteFeature,
            [lngLat.lng, lngLat.lat]
          );
          checkpointData.elevation = getElevationAtPoint(
            data?.routes as RouteFeature,
            [lngLat.lng, lngLat.lat]
          );
