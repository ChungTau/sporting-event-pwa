...
export const createCustomMarker = (draggable:boolean = true) => {
...
export const createPartiMarker = ({ name, image }:{name:string, image:string}) => {
    const pinImage = document.createElement('img');
    pinImage.src = image; // Path to the participant's image
    pinImage.style.width = '40px';
    pinImage.style.height = '40px';
    pinImage.style.borderRadius = '50%'; // Makes the image circular
    pinImage.style.border = '2px solid white'; // Adds a white border around the image
    pinImage.style.objectFit = 'cover'; // Ensures the image covers the area without distortion
    pinImage.style.objectPosition = 'center';
    
    const nameLabel = document.createElement('div');
    nameLabel.textContent = name;
    nameLabel.style.textAlign = 'center'; // Centers the name text below the image
    nameLabel.style.width = '100%'; // Ensures the text aligns under the image
    nameLabel.style.color = 'white'; // Sets text color
    nameLabel.style.fontSize = '12px'; // Adjust font size as needed
    nameLabel.style.marginTop = '4px'; // Space between image and name
    nameLabel.style.padding = '2px 6px'; // Padding around text
    nameLabel.style.background = 'rgba(0, 0, 0, 0.75)'; // Semi-transparent black background
    nameLabel.style.borderRadius = '4px'; // Optional: rounded corners

    const triangle = document.createElement('div');
    triangle.style.width = '0';
    triangle.style.height = '0';
    triangle.style.borderLeft = '5px solid transparent'; // Creates left half of triangle
    triangle.style.borderRight = '5px solid transparent'; // Creates right half of triangle
    triangle.style.borderTop = '5px solid rgba(0, 0, 0, 0.75)'; // Triangle's color matches the label
    triangle.style.margin = 'auto'; // Center the triangle under the label

    const markerEl = document.createElement('div');
    markerEl.appendChild(pinImage);
    markerEl.appendChild(nameLabel);
    markerEl.appendChild(triangle); // Adds the triangle below the label
    markerEl.style.display = 'flex';
    markerEl.style.flexDirection = 'column'; // Aligns the image and text in a column
    markerEl.style.alignItems = 'center'; // Centers the content

    return new mapboxgl.Marker({
        element: markerEl,
        draggable: false,
        offset: [0, -40] // Adjust the offset to position the marker correctly
    });
}
