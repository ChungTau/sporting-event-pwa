...
function TimeField(props: AriaTimeFieldProps<TimeValue>) {
    ...
    // Make sure 'aria-label' is used if no other labeling is provided
    const effectiveFieldProps = {
      ...fieldProps,
    };
...
const TimePicker = React.forwardRef<
  HTMLDivElement,
  Omit<TimeFieldStateOptions<TimeValue>, 'locale'>& { ariaLabel?: string }
>((props, forwardedRef) => {
      ...
      <Popover open={props.isOpen} onOpenChange={props.onOpenChange}>
        ...
        <PopoverContent ref={contentRef} className="w-full">
          <div {...dialogProps} className="space-y-3">
            ...
            {state.hasTime && <TimeField aria-label='timefield' value={state.timeValue} onChange={state.setTimeValue} />}
          </div>
        </PopoverContent>
      </Popover>
